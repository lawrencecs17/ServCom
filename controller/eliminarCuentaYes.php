<?php
	...
	if($_POST["id"])
	{
		$cuenta->delete($_POST["id"]);
		$cuenta=$cuenta->findById($_POST["id"]); 
		
		if($cuenta->getStatus()==-1)
		{
			$pnlcontent = new Panel("../view/aviso.html");
			$pnlcontent->add("aviso","Cuenta desactivada exitosamente");
		}
		else
		{
			$pnlcontent = new Panel("../view/error.html");
			$pnlcontent->add("aviso","Ha ocurrido un error, por favor intente en otro momento A)".$cuenta->getStatus());
		}
	}
	else
	{
		$pnlcontent = new Panel("../view/error.html");
		$pnlcontent->add("aviso","Ha ocurrido un error, por favor intente en otro momento B)".$cuenta->getStatus());		
	}
